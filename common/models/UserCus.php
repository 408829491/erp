...
class UserCus extends ActiveRecord implements IdentityInterface
{
    ...
    public function rules()
    {
        return [
			...
			[['status', 'r_id', 'sex', 'created_at', 'last_login_date', 'integral', 'updated_at','is_check','is_deleted','sale_man_id','is_pay_on'], 'integer'],
            [['username'], 'required'],
            ['username', 'string', 'min'=>6, 'max' => 50],
			['nickname', 'string', 'max' => 32],
            ['email', 'email'],
			[['openid', 'head_pic', 'email','business_license','address'], 'string', 'max' => 255],
			['mobile', 'filter', 'filter' => 'trim'],
			[['mobile'], 'string', 'max' => 11],
			['mobile','match','pattern'=>'/^[1][34578][0-9]{9}$/'],
			['mobile', 'unique'],
			['uid', 'number'],
			[['access_token','delivery_time'], 'string', 'max' => 100],
            [['username','email'],'unique'],
			[['balance'], 'number'],
			[['created_address', 'last_login_address'], 'string', 'max' => 200],
            [['created_ip', 'last_login_ip','receive_mobile'], 'string', 'max' => 15],
            [['invite_code','area_name','sale_man','c_type','line_name','source'], 'string', 'max' => 50],
            [['shop_name'], 'string', 'max' => 150],
            [['device_addr', 'device_pwd', 'print_num', 'print_pwd','stock_name'], 'string', 'max' => 50],
            [['sort_id'],'string'],
        ];
    }
    ...
    {
        return $this->hasMany(CusOrder::className(), ['user_id' => 'id']);
    }
