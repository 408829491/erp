...
<script>
    layui.config({
        ...
        table.render({
            elem: '#order-list'
            ,url: '/admin/stock-inventory-check/list'
            ,toolbar: '#order-list-toolbarDemo'
            ,title: '盘点单'
            ,cols: [[
                 {field:'id', title:'ID', width:80, fixed: 'left', unresize: true, sort: true}
                ,{field:'check_no', title:'盘点单号', width:235}
                ,{field:'create_time', title:'盘点时间', width:230,sort: true}
                ,{field:'store_name', title:'仓库', sort: true}
                ,{field:'loss_price', title:'盘损金额'}
                ,{field:'overflow_price', title:'盘溢金额'}
            ...
            switch(obj.event) {
                case 'getCheckData':
                    var windows = layer.open({
                        type: 2,
                        content: '/admin/stock-inventory-check/create',
                        area: ['800px', '830px'],
                        title: '新增手工单据',
                        maxmin: true,
            ...
            switch(obj.event){
                case 'edit':
                    var windows_update = layer.open({
                        type: 2,
                        content: '/admin/stock-inventory-check/update?id=' + obj.data.id,
                        area: ['800px', '830px'],
                        title:'编辑',
                        maxmin: true,
                        btn: ['保存', '取消'],yes: function(index, layero){
                ...
                case 'detail':
                    var window_detail = layer.open({
                        type: 2,
                        content: '/admin/stock-inventory-check/view?id='+ obj.data.id,
                        area: ['800px', '830px'],
                        title:'详情',
                        maxmin: true,
                        btn: ['返回']
                    ...
                    break;
                case 'close':
                    layer.confirm('确认要删除该单据吗?', function(index){
                        $.get({
                            type:"POST",
                            url:"/admin/stock-inventory-check/close?id="+obj.data.id,
                            dataType:"json",
