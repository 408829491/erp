...
<script type="text/html" id="isShow">
    <input type="checkbox" name="is_show" value="{{d.is_show}}" lay-skin="switch" lay-text="启用|关闭" lay-filter="switch-check" {{ d.is_show == 1 ? 'checked' : '' }}>
</script>
...
<script>
    layui.config({
        ...
        var $ = layui.$, active = {
            batchdel: function () {
                ...
                layer.confirm('确定删除吗？', function (index) {
                    ...
                    admin.req({
                        type: "post"
                        , url: '/admin/cus-commodity-category-by-store-manager/delete'
                        , dataType: "json"
                        , cache: false
                        , data: field
                        , done: function () {
            ...
            add: function () {
                var index=layer.open({
                    type: 2
                    , title: '添加'
                    , content: '/admin/cus-commodity-category-by-store-manager/create?pid=0'
                    , maxmin: true
                    , area: ['650px', '400px']
                    , btn: ['确定', '取消']
        ...
        var renderTable = function () {
            ...
            treetable.render({
                ...
                cols: [[
                    ...
                    , {field: 'is_show', title: '小程序中是否显示', width: 150, templet: '#isShow'}
        ...
        table.on('tool(list)', function (obj) {
            ...
            if (obj.event === 'subAdd') {
                layer.open({
                    type: 2
                    , title: '添加'
                    , content: '/admin/cus-commodity-category-by-store-manager/create?pid=' + data.id
                    , maxmin: true
                    , area: ['500px', '280px']
                    , btn: ['确定', '取消']
                    , yes: function (index, layero) {
                ...
                layer.open({
                    type: 2
                    , title: '编辑'
                    , content: '/admin/cus-commodity-category-by-store-manager/update?id=' + data.id
                    , maxmin: true
                    , area: area
                    , btn: ['确定', '取消']
                    , yes: function (index, layero) {
                        ...
                        iframeWindow.layui.form.on('submit(layuiadmin-app-form-edit)', function (data1) {
                            ...
                            admin.req({
                                type: "post"
                                , url: '/admin/cus-commodity-category-by-store-manager/edit'
                                , dataType: "json"
                                , cache: false
                                , data: field
                                , done: function () {
        ...
        form.on('switch(switch-check)', function(data){
            ...
            $.get({
                type: "get",
                url: "/admin/cus-commodity-category-by-store-manager/change-status?id=" + id,
                dataType: "json",
