        ...
        <div class="layui-input-block">
            <select name="sys_operating_time" >
                ...
                <?php foreach ($delivery_time as $item) : ?>
                    <option value="<?=$item->time_range?>" <?php if($item->time_range==$model->sys_operating_time):?>selected<?php endif;?>><?=$item->time_range?></option>
                <?php endforeach; ?>
            </select>
        </div>
    </div>
...
<script>
    layui.config({
        ...
        form.on('submit(layuiadmin-app-form-submit)', function (data) {
            ...
            admin.req({
                type: "post"
                , url: '/admin/customer-type/save?id=<?=$model->id?>'
                , dataType: "json"
                , cache: false
                , data: data.field
                , done: function (e) {
