...
<script>
    layui.config({
        ...
        table.render({
            elem: '#order-list'
            ,url: '/admin/cus-order/order-list?storeId=<?= $storeId ?>'
            ,toolbar: '#order-list-toolbarDemo'
            ,title: '用户数据表'
            ,cols: [[
                 {field:'id', title:'ID', width:80, fixed: 'left', unresize: true, sort: true}
                ,{field:'order_no', title:'订单号', width:235}
                ,{field:'nick_name', title:'订购用户', width:230,sort: true}
                ,{field:'price', title:'下单金额', width:130, sort: true}
                ,{field:'delivery_date', title:'发货日期',width:130}
                ,{field:'status_text', title:'订单状态', width:100, sort: true}
                ,{field:'pay_text', title:'付款状态', width:110}
                ,{field:'source_txt', title:'订单来源', width:100, sort: true}
            ...
            switch(obj.event) {
                case 'getCheckData':
                    var windows = layer.open({
                        type: 2,
                        content: '/admin/cus-order/create',
                        area: ['800px', '830px'],
                        title: '新增手工订单',
                        maxmin: true,
            ...
            switch(obj.event){
                case 'edit':
                    var windows_update = layer.open({
                        type: 2,
                        content: '/admin/cus-order/update?id=' + obj.data.id,
                        area: ['800px', '830px'],
                        title:'编辑订单',
                        maxmin: true,
                ...
                case 'detail':
                    var window_detail = layer.open({
                        type: 2,
                        content: '/admin/cus-order/view?id='+ obj.data.id,
                        area: ['800px', '830px'],
                        title:'订单详情',
                        maxmin: true,
                        btn: ['返回']
                ...
                case 'refund':
                    layer.open({
                        type: 2,
                        content: '/admin/cus-order/refund?id=' + obj.data.id,
                        area: ['800px', '630px'],
                        title:'退货',
                        maxmin: true,
                ...
                case 'approval':
                    var windows_approval = layer.open({
                        type: 2,
                        content: '/admin/cus-order/approval?id=' + obj.data.id,
                        area: ['800px', '830px'],
                        title:'核算订单',
                        maxmin: true,
                ...
                case 'copy':
                    var windows_copy = layer.open({
                        type: 2,
                        content: '/admin/cus-order/update?type=copy&id=' + obj.data.id,
                        area: ['800px', '830px'],
                        title:'复制订单',
                        maxmin: true,
                ...
                case 'finish':
                    layer.confirm('确认要完成该订单吗?', function(index){
                        $.get({
                            type:"POST",
                            url:"/admin/cus-order/finish?order_id="+obj.data.id,
                            dataType:"json",
                ...
                case 'close':
                    layer.confirm('确认要关闭该订单吗?', function(index){
                        $.get({
                            type:"POST",
                            url:"/admin/cus-order/close-order?order_id="+obj.data.id,
                            dataType:"json",
                            success:function(data){
                                if(data.code=='200'){
                                    table.reload('order-list');
