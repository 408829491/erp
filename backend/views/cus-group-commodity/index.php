        ...
        <div class="layui-card-body">
            ...
            <script type="text/html" id="status">
                {{#
                    ...
                    var curDate = Date.parse(year + "-" + (month > 9 ? month : "0" + month) + "-" + (date2 > 9 ? date2 : "0" + date2));

                    var activityDate = Date.parse(d.date);
                    if(d.is_close == 1 || activityDate < curDate){
                }}
                已结束
                {{#
                    } else if(activityDate == curDate) {
                }}
                已开始
                {{#  } else { }}
                未开始
        ...
        var $ = layui.$, active = {
            batchdel: function () {
                ...
                layer.confirm('确定删除吗？', function (index) {
                    ...
                    admin.req({
                        type: "post"
                        , url: '/admin/cus-group-commodity/delete'
                        , dataType: "json"
                        , cache: false
                        , data: field
                        , done: function () {
            ...
            add: function () {
                var index=layer.open({
                    type: 2
                    , title: '添加'
                    , content: '/admin/cus-group-commodity/create'
                    , maxmin: true
                    , area: ['600px', '300px']
                    , btn: ['确定', '取消']
        ...
        table.on('tool(list)', function (obj) {
            ...
            if (obj.event === 'del') {
                ...
                var index = layer.open({
                    type: 2
                    , title: '编辑'
                    , content: '/admin/cus-group-commodity/update?id=' + data.id
                    , maxmin: true
                    , area: ['500px', '250px']
                    , btn: ['确定', '取消']
                    , yes: function (index, layero) {
                ...
                admin.req({
                    type: "post"
                    , url: '/admin/cus-group-commodity/close'
                    , dataType: "json"
                    , cache: false
                    , data: {
                        id: data.id
