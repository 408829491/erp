<div class="layui-fluid">
    ...
    <script>
        layui.config({
            ...
            table.render({
                elem: '#list'
                ,url: '/admin/cus-delivery-driver/get-index-data?storeId=<?= $storeId ?>'
            ...
            table.on('toolbar(list)', function(obj){
                switch(obj.event) {
                    case 'getCheckData':
                        layer.open({
                            type: 2,
                            content: '/admin/cus-delivery-driver/create?storeId=<?= $storeId ?>',
                            area: ['800px', '430px'],
                            title: '新增司机',
                            maxmin: true,
            ...
            table.on('tool(list)', function(obj){
                switch(obj.event){
                    case 'edit':
                        layer.open({
                            type: 2,
                            content: '/admin/cus-delivery-driver/update?id=' + obj.data.id,
                            area: ['800px', '430px'],
                            title:'编辑',
                            maxmin: true,
                            btn: ['保存', '取消'],yes: function(index, layero){
                    ...
                    case 'detail':
                        layer.open({
                            type: 2,
                            content: '/admin/cus-delivery-driver/view?code='+ obj.data.invitation_code,
                            area: ['1080px', '830px'],
                            title:'详情',
                            maxmin: true,
                            btn: ['返回']
                    ...
                    case 'del':
                        layer.confirm('确认要删除该司机吗?', function(index){
                            $.get({
                                type:"POST",
                                url:"/admin/cus-delivery-driver/del?id="+obj.data.id,
                                dataType:"json",
