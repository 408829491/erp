<form class="layui-form" style="padding: 20px 30px 0 0;">
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" name="nickname"  value="<?=$model->nickname?>" placeholder="请输入司机姓名" autocomplete="off" class="layui-input" lay-verify="required">
        </div>
    </div>
    <div class="layui-form-item">
        <label class="layui-form-label"><span style="color: #ff6d6d;margin-right: 5px;">*</span>电话号码</label>
        <div class="layui-input-block">
            <input type="text" name="mobile"  value="<?=$model->mobile?>" required placeholder="请输入电话号码" autocomplete="off" class="layui-input" lay-verify="required">
        </div>
    </div>
    <div class="layui-form-item">
        <label class="layui-form-label"><span style="color: #ff6d6d;margin-right: 5px;">*</span>用户名</label>
        <div class="layui-input-block">
            <input type="text" name="username"  value="<?=$model->username?>" placeholder="司机帐号为手机号，不能修改" autocomplete="off" class="layui-input layui-disabled" lay-verify="required">
        </div>
    </div>
    <div class="layui-form-item">
...
<script>
    layui.config({
        ...
        form.on('submit(layuiadmin-app-form-submit)', function (data) {
            ...
            admin.req({
                type: "post"
                , url: '/admin/cus-delivery-driver/save?id=<?=$model->id?>'
                , dataType: "json"
                , cache: false
                , data: data.field
                , done: function (e) {
