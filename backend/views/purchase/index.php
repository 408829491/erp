            ...
            <div class="layui-card">
                <div class="layui-tab layui-tab-brief" lay-filter="component-tabs">
                    <ul class="layui-tab-title">
                        <li lay-data="0"  lay-data="1">全部状态</li>
                        <li class="layui-this">待采购</li>
                        <li lay-data="2">部分收货</li>
                        <li lay-data="3">全部收货</li>
                ...
                <div class="layui-form layui-card-header layuiadmin-card-header-auto" lay-filter="layui-form">
                    <div class="layui-form-item">
                        ...
                        <div class="layui-inline">
                            <div class="layui-input-inline">
                                <input type="text" name='plan_date' class="layui-input" id="test-send-date-range-date" placeholder="请选择计划交货日期">
                            </div>
                        </div>
                        <div class="layui-inline">
                                <input type="text" name="keyword" placeholder="输入采购员/单号" autocomplete="off" class="layui-input" id="commodity" style="width: 300px;">
                        </div>
                        <div class="layui-inline">
                        ...
                        <div class="layui-form layui-inline" lay-filter="layui-form" style="position:absolute;right:10px;">
                            <input type="checkbox" id="printPrice" lay-skin="primary" title="是否打印采购金额">
                        </div>
            ...
            <div class="layui-card">
                <div class="layui-card-body">
                    ...
                    <script type="text/html" id="order-list-barDemo">
                        ...
                        <a class="layui-btn layui-btn-xs" lay-event="export" style="background-color: #8198cf">导出</a>
                        <a class="layui-btn layui-btn-xs tpl_print_view_pur" lay-event="print" data-id="{{d.id}}" style="background-color: #77CF20">打印</a>
                        <a class="layui-btn layui-btn-xs" lay-event="finish" style="background-color: #77CF20">收货</a>
...
<script>
    layui.config({
        ...
        table.render({
            elem: '#order-list'
            ,url: '/admin/purchase/purchase-list'
            ...
            ,cols: [[
                ...
                ,{field:'price', title:'采购金额',width:90}
                ,{field:'purchase_price', title:'收货金额',width:90}
                ,{field:'author', title:'制单人', width:80}
                ,{field:'status_text', title:'状态', width:100}
                ,{field:'plan_date', title:'计划交货日期', width:120}
                ,{field:'purchase_num', title:'总采购项' ,width:88}
                ,{field:'procured_num', title:'已采购项' ,width:88}
                ,{field:'driver_name', title:'采购进度条', width:120,templet:'#progressTpl'}
                ,{fixed: 'right', title:'操作', toolbar: '#order-list-barDemo',width:308}
            ]]
            ,page: true
            ,done: function(){
                element.render();
            ...
            switch(obj.event){
                case 'edit':
                    var windows_update = layer.open({
                        type: 2,
                        content: '/admin/purchase/update?id=' + obj.data.id,
                        area: ['800px', '830px'],
                        title:'编辑采购单',
                        maxmin: true,
                        btn: ['保存采购单', '取消'],yes: function(index, layero){
                            var submit = layero.find('iframe').contents().find("#layuiadmin-app-form-submit");
                            submit.click();
                        }
                    });
                ...
                case 'detail':
                    var window_detail = layer.open({
                        type: 2,
                        content: '/admin/purchase/view?id='+ obj.data.id,
                        area: ['800px', '830px'],
                        title:'采购单详情',
                        maxmin: true,
                        btn: ['返回']
                ...
                case 'finish':
                    layer.confirm('确认要收货吗?', function(index){
                        $.get({
                            type:"POST",
                            url:"/admin/purchase/finish?purchase_id="+obj.data.id,
                            dataType:"json",
                            success:function(data){
                                if(data.code=='200'){
                                    table.reload('order-list');
                                    layer.msg('收货已完成！', {icon: 1});
                                }else{
                    ...
                    break;
                case 'close':
                    layer.confirm('确认要关闭该采购单吗?', function(index){
                        $.get({
                            type:"POST",
                            url:"/admin/purchase/close-purchase?purchase_id="+obj.data.id,
                            dataType:"json",
                            success:function(data){
                                if(data.code=='200'){
                                    table.reload('order-list');
                    ...
                    break;
                case 'export':
                    document.location.href = '/admin/purchase/export-purchase?id='+obj.data.id;
                    break;
            }
        });
