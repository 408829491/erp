<form class="layui-form" style="padding: 20px 30px 0 0;">
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" name="name"  value="<?=$model->nickname?>" placeholder="请输入采购员姓名" autocomplete="off" class="layui-input" lay-verify="required" >
        </div>
    </div>
    <div class="layui-form-item">
        <label class="layui-form-label"><span style="color: #ff6d6d;margin-right: 5px;">*</span>手机号码</label>
        <div class="layui-input-block">
            <input type="text" name="phone"  value="<?=$model->username?>" placeholder="手机号码为用户帐号，不能修改" autocomplete="off" class="layui-input" lay-verify="required|phone">
        </div>
    </div>
    <div class="layui-form-item">
        <label class="layui-form-label">密码</label>
        <div class="layui-input-block">
            <input type="text" name="password"  value="" placeholder="如果不修改密码，请留空" autocomplete="off" class="layui-input">
        </div>
    </div>
...
<script>
    layui.config({
        ...
        form.on('submit(layuiadmin-app-form-submit)', function (data) {
            ...
            admin.req({
                type: "post"
                , url: '/admin/purchase-buyer/save?id=<?=$model->id?>'
                , dataType: "json"
                , cache: false
                , data: data.field
                , done: function (e) {
