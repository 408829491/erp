...
<script>
    layui.config({
        ...
        var $ = layui.$, active = {
            batchdel: function () {
                ...
                layer.confirm('确定删除吗？', function (index) {
                    ...
                    admin.req({
                        type: "post"
                        , url: '/admin/cus-store-slideshow-by-store-manager/delete'
                        , dataType: "json"
                        , cache: false
                        , data: field
                        , done: function () {
            ...
            add: function () {
                var index=layer.open({
                    type: 2
                    , title: '添加'
                    , content: '/admin/cus-store-slideshow-by-store-manager/create'
                    , maxmin: true
                    , area: ['800px', '300px']
                    , btn: ['确定', '取消']
        ...
        table.render({
            elem: '#list'
            , url: '/admin/cus-store-slideshow-by-store-manager/index-data'
            , cols: [[
        ...
        table.on('tool(list)', function (obj) {
            ...
            if (obj.event === 'view') {
                ...
                var index = layer.open({
                    type: 2
                    , title: '编辑'
                    , content: '/admin/cus-store-slideshow-by-store-manager/update?id=' + data.id
                    , maxmin: true
                    , area: ['800px', '300px']
                    , btn: ['确定', '取消']
                    , yes: function (index, layero) {
                        ...
                        iframeWindow.layui.form.on('submit(layuiadmin-app-form-edit)', function (data1) {
                            ...
                            admin.req({
                                type: "post"
                                , url: layui.setter.reqUrlBase + '/admin/cus-store-slideshow-by-store-manager/edit'
                                , dataType: "json"
                                , cache: false
                                , data: field
                                , done: function () {
