...
class CusSeckillForm {
    ...
    public function saveData($model, $subList) {
        $transaction  = CusSeckill::getDb()->beginTransaction();
        ...
        try {
            ...
            if (sizeof($subList) != 0) {
                foreach ($subList as $value) {
                    $model1 = new CusSeckillCommodity();
                    $model1->attributes = $value;
                    $model1->store_id = $model->store_id;
                    $model1->cus_seckill_id = $model->id;
                    if (!$model1->validate()) {
    ...
    public function editData($model, $subList) {
        ...
        try {
            ...
            // 删除子表
            CusSeckillCommodity::deleteAll(['cus_seckill_id' => $model->id]);
            // 保存子表
