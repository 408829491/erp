 ...
 * @property int $pay_way 支付方式，0余额，1微信
 ...
 * @property int $driver_id 配送员ID
 * @property string $driver_name 配送员姓名
 * @property bool $driver_is_fault 配送员标记异常订单
 * @property string $delivery_date 发货日期
 * @property string $delivery_time_detail 发货时间段
 * @property int $delivery_time_id 发货时间段ID
 * @property int $is_send_to_home 是否配送到家（0到店自取，1配送到家）
 ...
 * @property string $user_id 下单客户ID
...
class CusOrder extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['store_id', 'pay_way', 'pay_time', 'pay_record_no_expire', 'status', 'line_sequence', 'driver_id', 'delivery_time_id', 'is_send_to_home', 'user_id', 'close_time', 'close_type', 'coupon_id', 'is_audit', 'audit_time', 'audit_user_id', 'source', 'return_num', 'create_time'], 'integer'],
            [['order_no', 'delivery_date', 'delivery_time_detail', 'user_name', 'create_time'], 'required'],
            [['price', 'pay_price', 'total_profit', 'quantity', 'address_lng', 'address_lat', 'freight_price', 'reduction_price', 'return_price'], 'number'],
            [['is_pay'], 'string'],
            [['driver_is_fault'], 'boolean'],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'driver_is_fault' => 'Driver Is Fault',
