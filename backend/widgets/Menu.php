...
class Menu extends \yii\widgets\Menu
{
     ...
     * @inheritdoc
     */
    public $linkTemplate = '<a lay-href="{url}">{icon} {label}</a>';
    /**
     * @inheritdoc
    ...
    {
        if (isset($item['items'])) {
            $labelTemplate = '<a lay-href="{url}">{icon} {label} <span class="pull-right-container"></span></a>';
		...
		if(substr($item['icon'],0,2)=='&#'){
			$replacements = [
				...
				'{url}' => isset($item['url'][0]) ? 'lay-href="'.Url::to($item["url"]).'"' : 'javascript:void(0);',
			];
		}else{
			$replacements = [
				'{label}' => strtr($this->labelTemplate, ['{label}' => $item['label'],]),
				'{icon}' => empty($item['icon']) ? $this->defaultIconHtmlIcon: '<i class="'.$item['icon'].' "></i> ',
				'{url}' => isset($item['url'][0]) ? Url::to($item["url"]):'javascript:;',
			];
		}
