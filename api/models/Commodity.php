...
class Commodity extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['type_first_tier_id', 'type_id', 'is_online', 'sequence', 'category_id', 'category_id2', 'order_quantity', 'max_quantity', 'provider_id', 'agent_id', 'parent_id', 'is_process', 'hide', 'channel_type', 'allow_change_channel', 'is_time_price', 'is_sell_stock', 'product_line_id', 'unit_change_disabled', 'create_time', 'modify_time', 'delete_time'], 'integer'],
            [['price', 'in_price', 'unit_num', 'loss_rate', 'sell_stock'], 'number'],
            [['rule_status', 'is_active', 'is_rough', 'unit_convert', 'status'], 'string'],
            [['name', 'category', 'tag', 'notice'], 'string', 'max' => 255],
            [['unit', 'commodity_code', 'pinyin', 'brand', 'durability_period'], 'string', 'max' => 50],
            [['summary', 'pic'], 'string', 'max' => 999],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'pic' => 'Pic',
            'activity_price' => '活动价格(辅助)'
        ];
    }

    // 添加新字段
    public function attributes() {
        return array_merge(parent::attributes(), ['activity_price']);
    }
}
